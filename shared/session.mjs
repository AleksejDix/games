...
export function createSession({
  ...
  // seed, which is half of what a replay is. The other half, the moves,
  // starts here as an empty list the engines append to. This is the one
  // place the shell may roll Math.random: to name the world, never to
  // run it.
  let recording;
  ...
  const session = {
    ...
    get recording() {
      return recording; // { seed, options, moves } — the game as data, so far
    },
    settings,
    dispatch,
    newGame,
